    ...
    return SizedBox(
      ...
      child: LineChart(
        LineChartData(
            ...
            leftTitles: AxisTitles(
              sideTitles: SideTitles(
                ...
                getTitlesWidget: (v, _) => Text(
                  v.toInt().toString(),
                  style: TextStyle(color: cs.outline, fontSize: SizeConfig.w(10)),
                ),
              ),
            ),
          ...
          minX: 0, maxX: (data.length - 1).toDouble(), minY: 0, maxY: 40,
          lineTouchData: LineTouchData(
            enabled: true,
            touchTooltipData: LineTouchTooltipData(
              tooltipBgColor: cs.surfaceContainer.withValues(alpha: 0.95),
              tooltipBorder: BorderSide(color: cs.primary.withValues(alpha: 0.3)),
              tooltipRoundedRadius: SizeConfig.w(8),
              getTooltipItems: (spots) => spots.map((s) {
                final i = s.x.toInt();
                return LineTooltipItem(
                  '${data[i].month}\n${s.y.toInt()} cars',
                  TextStyle(color: cs.primary, fontWeight: FontWeight.w600, fontSize: SizeConfig.w(12)),
                );
              }).toList(),
            ),
          ),
          lineBarsData: [
            LineChartBarData(
              ...
              color: cs.primary, barWidth: 3, isStrokeCapRound: true,
              dotData: FlDotData(
                show: true,
                getDotPainter: (_, __, ___, ____) => FlDotCirclePainter(
                  radius: SizeConfig.w(4), color: cs.primary,
                  strokeWidth: 2, strokeColor: cs.surface,
                ),
              ),
